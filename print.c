...
void printTypeExpression(typeExpressionRow* row) {
    if(row == NULL)
        printf("***");
    else if(row -> typeVariable == ERROR)
        return;
    else if(row -> typeVariable == PRIMITIVE)
        printf("(TYPE = %d) (RECTARRAY TYPE = %d) (ELEMENT TYPE = %d)", row -> typeVariable, row -> rectArrayType, row -> expression.primitive);
    else if(row -> typeVariable == RECTARRAY) {
        printf("(TYPE = %d) (RECTARRAY TYPE = %d) (DIM = %d) (RANGES = ", row -> typeVariable, row -> rectArrayType, row -> expression.rectArray.dimensions);
        ...
        printf("(TYPE = %d) (RECTARRAY TYPE = %d) (DIM = %d) (R1 = %d %d) (R2 = ", row -> typeVariable, row -> rectArrayType, row -> expression.jaggedArray.dimensions, row -> expression.jaggedArray.R1_range[0], row -> expression.jaggedArray.R1_range[1]);
